...
def cmd_cd_logic(args: list[str], current_node: dict) -> None:
    """Логика смены текущей директории."""
    if not args:
        current_path.clear()
        return
...
def cmd_head_logic(args: list[str], current_node: dict) -> None:
    """Логика вывода первых строк файла."""
    if not args:
...
def execute_command(command: str, args: list[str]) -> None:
    ...
    if command == CMD_LS:
        ...
        cmd_cd_logic(args, current_node)
        ...
        cmd_head_logic(args, current_node)
    else:
